...
bool PubSubClient::connect(const char* id) { return connect(id, NULL, NULL, 0, 0, 0, 0); }
...
#define MQTT_HEADER_VERSION_LENGTH 7
            ...
            while (!_client->available()) {
                unsigned long t = FruityHal::GetRtcMs();
            ...
            if (len == 4) {
                if (buffer[3] == 0) {
                    lastInActivity = FruityHal::GetRtcMs();
...
bool PubSubClient::readByte(uint8_t* result) {
    uint32_t previousMillis = FruityHal::GetRtcMs();
    while (!_client->available()) {
        uint32_t currentMillis = FruityHal::GetRtcMs();
...
uint16_t PubSubClient::readPacket(uint8_t* lengthLength) {
    ...
    *lengthLength = len - 1;
    ...
    for (uint16_t i = start; i < length; i++) {
        if (!readByte(&digit)) return 0;
        if (this->stream) {
            if (isPublish && len - *lengthLength - 2 > skip) { this->stream->write(digit); }
        }
        if (len < MQTT_MAX_PACKET_SIZE) { buffer[len] = digit; }
        len++;
    }
...
bool PubSubClient::publish_P(const char* topic, const uint8_t* payload, unsigned int plength, bool retained) {
    ...
    for (i = 0; i < plength; i++) { rc += _client->write((char)*(payload + i)); }

    lastOutActivity = FruityHal::GetRtcMs();
...
bool PubSubClient::write(uint8_t header, uint8_t* buf, uint16_t length) {
    ...
    buf[4 - llen] = header;
    for (int i = 0; i < llen; i++) { buf[5 - llen + i] = lenBuf[i]; }
    ...
    return result;
#else
    uint8_t* send = buf + (4 - llen);

    rc = _client->write(send, length + 1 + llen);
    lastOutActivity = FruityHal::GetRtcMs();
    return (rc == 1 + llen + length);
#endif
}

bool PubSubClient::subscribe(const char* topic) { return subscribe(topic, 0); }
